# Brain 知识库

> 这里是个人知识管理系统，采用"地图而非手册"的设计理念。
> 详细内容请查看各目录下的文档。

---

## 目录结构

| 目录 | 用途 | 何时阅读 |
|------|------|----------|
| [tech/](#tech) | 编程语言、框架、架构原则 | 技术选型、代码实现 |
| [tools/](#tools) | Git、Docker、CI/CD 等工具 | 工具使用、工作流 |
| [domain/](#domain) | 行业概念、产品逻辑、业务术语 | 理解业务、需求分析 |
| [pitfalls/](#pitfalls) | 踩坑记录、问题排查 | 调试、避免重复犯错 |
| [learning/](#learning) | 读书笔记、课程总结 | 概念回顾、知识关联 |
| [adr/](#adr) | 架构决策记录 | 理解历史决策 |
| [recipes/](#recipes) | 操作手册、速查表 | 快速查找步骤 |
| [ideas/](#ideas) | 想法、灵感 | 记录突发灵感 |

---

## tech

编程语言、框架、设计模式、架构原则

*（目前为空，欢迎添加）*

---

## tools

Git、Docker、CI/CD、开发环境、效率工具

*（目前为空，欢迎添加）*

---

## domain

行业概念、产品逻辑、业务术语

*（目前为空，欢迎添加）*

---

## pitfalls

踩坑记录、问题排查、解决方案

*（目前为空，欢迎添加）*

---

## learning

读书笔记、课程总结、概念理解

### 文档列表

| 文件 | 标题 | 概要 |
|------|------|------|
| [harness-engineering.md](./learning/harness-engineering.md) | Harness Engineering: 利用 Codex 构建 Agent-First 世界 | OpenAI 团队 5 个月实验：完全用 Codex AI Agent 构建 100 万行代码产品的心得。核心：人类掌舵 Agent 执行、环境设计比写代码重要、给地图而非手册、严格架构边界等。 |
| [skill-design-principles.md](./learning/skill-design-principles.md) | Skill 设计原则：宪法式设计 | 来源：Anthropic "Building Effective Agents" + Block Engineering。核心：用「宪法」替代「 SOP 」，五个设计原则：二区架构、约束>步骤、写宪法不写建议、自我纠错机制、简约+渐进式披露。 |
| [skill-community-practice.md](./learning/skill-community-practice.md) | Skill 社区实践与思想哲学 | GitHub 549+ skills 生态汇总：官方 Anthropic 16+ skills、大厂官方 skills (Vercel/Stripe/Cloudflare/Expo)、社区思想哲学、安全警告、创建最佳实践。 |

---

## adr

架构决策记录（为什么选 A 不选 B）

*（目前为空，欢迎添加）*

---

## recipes

操作手册、速查表、杂项

*（目前为空，欢迎添加）*

---

## ideas

记录想法、灵感

*（目前只有目录，可直接添加内容）*

---

## 使用指南

1. **查找内容**：先看 AGENTS.md 地图，找到对应目录
2. **深入阅读**：进入目录，阅读具体文档
3. **添加内容**：按目录分类添加新笔记
4. **保持更新**：定期清理过时内容

---

## 贡献

欢迎添加新内容！按目录分类提交，保持地图同步更新。

---

*最后更新: 2026-03-20*
